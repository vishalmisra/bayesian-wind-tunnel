...
workspace is the hypothesis frame (in coordinates whose alignment
weight tying and training seed modulate; the causal structure beneath
is invariant), its contents are the posterior's
...
frame-specific damage is confined to the entry band. Routing substrate is not writer computation. Exploratory
cross-architecture runs separate the principle from the implementation: an LSTM and
a Mamba-style SSM learn the same Bayesian function and show the same
content/readout dissociation, but neither stores the substrate as a
frame-aligned residual workspace; the LSTM re-encodes it into private
recurrent coordinates, and Mamba carries it in scan state, where a
residual-stream positional lens cannot see it. The separation is
general; the coordinates are architectural; the frame-aligned workspace
is the transformer's implementation of the substrate.
\end{abstract}
...
P1 & \Jspace{} $=$ frame subspace & $\ge 3\times$ null; decode $\ge 90\%$ &
  \textbf{Pass} on the production run ($4.8$--$6.8\times$; controls at
  null; magnitude seed- and tying-dependent) \\
...
\bottomrule
\end{tabular}}
\caption{Preregistration scorecard. Status ledger: P1 passed under a
within-family frame-mode refinement (prereg head-pullback modes pass
only at $r{=}4$); P3's gate required balanced accuracy and ranks beyond
the prereg grid (strict prereg outcome at $r{\le}16$: fail); P2's
dichotomy and all of the rescue, corruption, dose-response, formation,
and cross-architecture results are exploratory follow-ups. The
prespecified protocol is committed verbatim
(\texttt{PREREGISTRATION.md}); its public commit postdates the first
runs, so it is an internal prespecification, not an externally
timestamped preregistration. Full grids and deviations are in the
repository.}
\label{tab:scorecard}
\end{table}
...
\paragraph{Lenses.} The J-lens is the Jacobian member of a family of
early-readout methods: the logit lens~\citep{nostalgebraist2020logit},
its learned refinement the tuned lens~\citep{belrose2023tuned}, and the
future lens~\citep{pal2023future}, which like the J-lens targets tokens
beyond the next position. What the Jacobian adds is locality: an
infinitesimal readout that measures what the future depends on rather
than what a fitted map can extract, which is the distinction our
entropy-axis result turns into a measurement.

\paragraph{Causal interventions and abstraction.} Our swaps and
rescues sit in the activation-patching lineage of causal mediation and
tracing~\citep{vig2020causal, meng2022rome, wang2023ioi}, with the
methodological cautions of \citet{zhang2024patching} in mind. The
identification framing is the concern of causal
abstraction~\citep{geiger2021causal} and distributed alignment
search~\citep{geiger2024das}, and the preregistered-hypothesis ethos is
causal scrubbing's~\citep{chan2022scrubbing}. Our P4 is an interchange
intervention scored against an \emph{exact} causal model --- the
analytic posterior --- which is the limit case those frameworks
approximate when the high-level model must itself be hypothesized.

\paragraph{Probing and its critique.} That decodable content need not
be used is established: amnesic probing~\citep{elazar2021amnesic}
demonstrated it with projection-removal, control tasks diagnose probe
overcapacity~\citep{hewitt2019control}, and the survey
of~\citet{belinkov2022probing} collects the genre; the removal
machinery itself descends from INLP~\citep{ravfogel2020inlp} and
LEACE~\citep{belrose2023leace}. Our contribution to this line is
calibration against ground truth: the inert-but-decodable quantity is
checked against an analytic posterior, and the lens's exclusion of it
is measured rather than argued. Our P3 probe-promiscuity result is the
same critique applied to ourselves.

\paragraph{Ground-truth settings.} Compiled transformers
(Tracr,~\citealt{lindner2023tracr}) and emergent world models
(Othello-GPT,~\citealt{li2023othello}) also compare internal structure
against known semantics. The wind tunnel's differentiator is that the
models are \emph{trained}, not compiled, and the ground truth is a
closed-form posterior rather than a board state, so calibration itself
is checkable to fractions of a bit.

\paragraph{Circuits, formation, and the nearest neighbor.} The
residual-stream framework and induction-head account are the
background picture~\citep{elhage2021framework, olsson2022induction};
ablation-based indispensability carries the self-repair caveat of the
Hydra effect~\citep{mcgrath2023hydra}, which our absolute
frame-projected write metric partially answers. That structure forms
before performance has precedent in grokking progress
measures~\citep{nanda2023grokking}. The closest single precedent to
routing-versus-content is the binding-ID result
of~\citet{feng2023binding}: content-independent binding subspaces
whose interchange redirects bindings. Our workspace stores one
direction per live hypothesis, in contrast to
superposition~\citep{elhage2022superposition}; whether that survives
$V \gg d$ is open. That in-context learning is implicitly Bayesian was
argued by~\citet{xie2022bayesian}; the wind tunnel makes it exact.

\paragraph{Recurrent and state-space interpretability.}
\citet{paulo2024rnns} find that transformer interpretability tools
largely transfer to Mamba and RWKV at scale, and \citet{sharma2024mamba}
edit factual associations in Mamba through residual interventions. Our
Mamba result does not contradict either: next-token readouts and many
behaviors remain residual-accessible; what our task isolates is
\emph{evidence transport}, which in this regime runs through scan
state. The reconciliation is a prediction: residual-stream methods
should succeed on SSMs exactly where content is locally materialized
and fail where it is carried in state.

\paragraph{Global workspace theory.} We use ``workspace''
operationally, following the production release, and make no contact
with global workspace theory
proper~\citep{baars1988cognitive, dehaene2011workspace} or its ML
instantiations~\citep{goyal2022workspace, vanrullen2021workspace}:
nothing here concerns broadcast, ignition, or access. If anything, P5
cautions against workspace-detection arguments in that tradition's
direction: the subspace can exist, correctly shaped, and confer
nothing, so detecting a workspace licenses no inference to function.
...
properties, measured causally, as amnesic
probing~\citep{elazar2021amnesic} first showed with projection
removal --- and the lens agrees: the entropy
...
weights themselves. One per-position parameter does exist, the learned
positional embeddings, so this is an empirical regularity rather than a
theorem: the positional pathway could in principle localize the
geometry, and measurably does not dominate the frame. ``Geometry global, writer computation
...
attention in these models keeps the substrate as a frame-aligned
positional subspace of the residual stream (an empirical regularity,
not a necessity: QK maps can absorb any re-basing, and the untied
control shows tying drives much of the token alignment); the LSTM keeps
the same content at layer boundaries in a learned private code; Mamba
keeps it in scan state, off the residual stream altogether. A practical implication for production
...
posterior, that the functional subspace the Jacobian identifies matches
the generative latent structure it should identify, up to the
coordinate contingencies of \cref{sec:crossarch}. Ground-truth
comparisons exist for probed representations (Othello-GPT,
\citealt{li2023othello}) and compiled circuits
(Tracr, \citealt{lindner2023tracr}); what we believe is new is the
narrower Jacobian-side statement: a use-weighted functional subspace
compared against an analytically known posterior structure, with the
two computed independently.
...
single-token equivalents is untested here. And of the production
definition's three workspace criteria (dense read/write, flexible
cross-task reuse, causal necessity), only the third is fully exercised:
cross-task reuse is untestable in a one-task model, and our
read-connectivity ratios are far below theirs.

\paragraph{Artifacts.} Code, per-cell grids, the verbatim
prespecification (\texttt{PREREGISTRATION.md}, with an honest note on
its dating), deviations, and all figures: \texttt{experiments/jlens/}
in the wind-tunnel repository, pinned against
\texttt{anthropics/jacobian-lens} \texttt{@581d3986}. The full study is reproducible in ${\sim}2$ GPU-hours
on one consumer GPU.
